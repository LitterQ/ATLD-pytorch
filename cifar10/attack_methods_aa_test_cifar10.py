...
class Attack_None(nn.Module):
    ...
    def forward(self, inputs, targets, attack=None, batch_idx=-1):
        ...
        #logits_pert,_,fea_nat = self.basic_net(x)
        #------------test--------------
        ...
        for idxxx in range(1):
            ...
            #logits_fea = self.discriminator(fea_nat)
            logits_fea, D_cla = self.discriminator(logits_pred_D)
            #logits_fea = self.discriminator(logits_pred_D)
            loss_D = adversarial_criterion(logits_fea, valid)
            #print(logits_fea)
            confidence = torch.nn.functional.softmax(D_cla, dim=1)
            #print(D_cla.shape)
            #print(confidence)
            mask = torch.Tensor(np.ones((x.size(0),1,1,1))).cuda()
            for index,qq in enumerate(logits_fea):
                #if qq < 0.51:
                #first,second = torch.topk(qq,2)
                #if 0.2<torch.max(qq) < 0.8:
                if 0.3 < qq < 0.7:
                #if first[0].float()-first[1].float() >= 0.44:
                    mask[index] = 0.5
                #elif qq >= 0.51:
...
class Attack_PGD(nn.Module):
    ...
    def forward(self,
        ...
        logits_pert = torch_model.forward(x)
        #logits_pert,_,fea_nat = self.basic_net(x)
        ...
        for idxxx in range(1):
            ...
            #print(x.grad)
            x_adv = x.data - self.step_size * 1 * 2.0  * mask  * torch.sign(x.grad.data)
            x_adv = torch.min(torch.max(x_adv, x - self.epsilon * 1.0),
                              x + self.epsilon * 1.0)
            ...
            logits_pert_imt = torch_model.forward(x.detach())

        return logits_pert, targets_prob.detach(), logits_pert_imt
