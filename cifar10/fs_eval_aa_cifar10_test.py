...
from models_new.wideresnet_aa import *
from models_new.dis3_new7_svhn import *
#from models_new.dis import *
import utils

from attack_methods_aa_test_cifar10 import Attack_None, Attack_PGD
...
def test(epoch, net):
    ...
    for batch_idx, (inputs, targets) in enumerate(iterator):
        ...
        outputs, _, outputs_imt = net(pert_inputs, targets, batch_idx=batch_idx)
        ...
        _, predicted_imt = outputs_imt.max(1)
        batch_size = targets.size(0)
        total += batch_size
        correct_num = predicted.eq(targets).sum().item()
        correct_num_imt = predicted_imt.eq(targets).sum().item()
        correct += correct_num
        correct_imt += correct_num_imt
        iterator.set_description(
            str(predicted.eq(targets).sum().item() / targets.size(0)))

        if batch_idx % 1 == 0:
            print(
                "step %d, duration %.2f, test  acc %.2f, avg-acc %.2f, imt acc %.2f, imt-avg %.2f"
                % (batch_idx, duration, 100. * correct_num / batch_size,
                   100. * correct / total, 100. * correct_num_imt / batch_size, 100. * correct_imt / total))
        torch.cuda.empty_cache()

    acc = 100. * correct / total
    acc_imt = 100. * correct_imt / total
    print('Val acc:', acc)
    print('IMT acc:', acc_imt)
    return acc
...
for attack_idx in range(attack_num):
    ...
    if args.attack_method == 'natural':
        ...
        net = Attack_None(basic_net, config_natural, discriminator, D_optimizer)
    elif args.attack_method.upper() == 'FGSM':
        print('-----FGSM adv mode -----')
        net = Attack_PGD(basic_net, config_fgsm, discriminator, D_optimizer)
    elif args.attack_method.upper() == 'PGD':
        print('-----PGD adv mode -----')
        net = Attack_PGD(basic_net, config_pgd, discriminator, D_optimizer)
    elif args.attack_method.upper() == 'CW':
        print('-----CW adv mode -----')
        net = Attack_PGD(basic_net, config_cw, discriminator, D_optimizer)
    else:
        raise Exception(
    ...
    if args.resume and args.init_model_pass != '-1':
        ...
        if not os.path.isdir(args.model_dir):
            ...
            #net_org = load_state_dict(checkpoint['net'])
            net.load_state_dict(checkpoint['net'])
            #start_epoch = checkpoint['epoch']
